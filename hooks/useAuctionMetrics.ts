...
const SUBGRAPH_URL =
  "https://gateway.thegraph.com/api/subgraphs/id/9BQvuZLRAHVaMomrczg5e66mfdcDmzLqZionmyAJ6f13";
...
export function useAuctionMetrics() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      console.log(
        "Raw ETH totalWinningBidsValue:",
        metrics.totalETHWinningBidsValue
      );
      console.log(
        "Raw ETH averageWinningBidValue:",
        metrics.averageETHWinningBidValue
      );
      console.log(
        "Raw QR totalWinningBidsValue:",
        metrics.totalQRWinningBidsValue
      );
      console.log(
        "Raw QR averageWinningBidValue:",
        metrics.averageQRWinningBidValue
      );
      ...
      try {
        totalETHWinningBidsValue =
          Number(metrics.totalETHWinningBidsValue) / 1e18;
      } catch (e) {
      ...
      try {
        averageETHWinningBidValue =
          Number(metrics.averageETHWinningBidValue) / 1e18;
      } catch (e) {
      ...
      try {
        totalQRWinningBidsValue =
          Number(metrics.totalQRWinningBidsValue) / 1e18;
      } catch (e) {
      ...
      try {
        averageQRWinningBidValue =
          Number(metrics.averageQRWinningBidValue) / 1e18;
      } catch (e) {
        console.error("Error parsing averageQRWinningBidValue:", e);
      }

      // Combined totals for display purposes
      let totalWinningBidsValue =
        totalETHWinningBidsValue + totalQRWinningBidsValue;
      const averageWinningBidValue =
        (totalETHWinningBidsValue + totalQRWinningBidsValue) /
        (parseInt(metrics.totalAuctions) || 1);

      // Fallback calculations if the values are still unreasonable
      if (totalWinningBidsValue > 1000000) {
        // Sanity check - if value is unreasonably large
        console.warn(
          "totalWinningBidsValue too large, using fallback calculation"
        );
      ...
      return {
        ...
        finalMinutesBidsPercentage: parseFloat(
          metrics.finalMinutesBidsPercentage
        ),
      };
    },
