...
const TEST_MODE_CONFIG = {
  ...
  MOCK_QR_URL:
    "https://warpcast.com/~/add-cast-action?url=https%3A%2F%2Fexample.com%2Ftest-signer",
    ...
    return (
      <div className="flex flex-col items-center space-y-4">
        <Button
          onClick={() =>
            testMode
              ? console.log("Test mode: would open", value)
              : frameSdk.redirectToUrl(value)
          }
...
export function LikesRecastsClaimPopup({
  ...
  const getOptionType = () => {
    ...
    if (permissions.length === 1 && permissions.includes("like"))
      return "likes";
    if (permissions.length === 1 && permissions.includes("recast"))
      return "recasts";
    if (
      permissions.length === 2 &&
      permissions.includes("like") &&
      permissions.includes("recast")
    )
      return "both";
  ...
  const mockData = {
    claimTokens: async () => ({
      signer_approval_url: TEST_MODE_CONFIG.MOCK_QR_URL,
    }),
    isPolling: false,
  ...
  const { walletAddress, hasSignerApproval } = testMode
    ? mockEligibility
    : eligibilityResults;
  ...
  useEffect(() => {
    if (testMode && isOpen) {
      toast.info(
        "🧪 Test Mode: Processing and QR approval auto-complete, other steps require manual interaction",
        {
          duration: 4000,
        }
      );
  ...
  useEffect(() => {
    ...
    async function checkFrameContext() {
      try {
        isFrameRef.current =
          (await frameSdk.isInMiniApp()) ||
          (await frameSdk.getContext()).client.clientFid == 309857;
      } catch {
      ...
      const autoSetupClaim = async () => {
        try {
          ...
          const response = await fetch(
            `/api/likes-recasts/user-permissions?fid=${context.user.fid}`
          );
          ...
          if (response.ok) {
            ...
            if (data.success && data.permissions) {
              ...
              console.log(
                `User has permissions for: ${userOptionType}, amount: ${userAmount}`
              );
  ...
  useEffect(() => {
    ...
    if (signerApprovalUrl && signerUuid) {
      console.log(
        "Got signer approval URL, transitioning to signer_approval state"
      );
      setClaimState("signer_approval");
    }
    ...
    if (signerUuid && claimState === "signer_approval") {
      const checkApproval = async () => {
        try {
          ...
          if (approved) {
            console.log(
              "Signer approved! Transitioning to signer_approved state"
            );
            setClaimState("signer_approved");
          }
        } catch (error) {
    ...
    if (signerUuid && claimState === "signer_approval" && !isPolling) {
      ...
      const checkApproval = async () => {
        try {
          ...
          if (approved) {
            console.log(
              "Final check: Signer approved! Transitioning to signer_approved state"
            );
            setClaimState("signer_approved");
          }
        } catch (error) {
  ...
  useEffect(() => {
    ...
    const checkClaimStatus = async () => {
      try {
        ...
        const response = await fetch(
          `/api/likes-recasts/claim-status?fid=${context.user.fid}`
        );
        const data = await response.json();
        ...
        if (data.success) {
          ...
          const statusUpdate = {
            ...
            hasClaimedLikes: claims.some(
              (c: { option_type: string }) => c.option_type === "likes"
            ),
            hasClaimedRecasts: claims.some(
              (c: { option_type: string }) => c.option_type === "recasts"
            ),
            hasClaimedBoth: claims.some(
              (c: { option_type: string }) => c.option_type === "both"
            ),
            isLoading: false,
          };
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      if ("ready_to_claim" in result && result.ready_to_claim) {
        ...
        toast.success(
          `${amount.toLocaleString()} $QR has been sent to your wallet.`,
          {
            style: {
  ...
  const handleManualClaim = async () => {
    ...
    toast.success(
      `${correctAmount.toLocaleString()} $QR has been sent to your wallet.`,
      {
        style: {
          background: "var(--primary)",
          color: "var(--primary-foreground)",
  ...
  const handleShare = async () => {
    ...
    const shareText = encodeURIComponent(
      `just earned my daily $QR reward for checking out today's winner @qrcoindotfun`
    );
  ...
  const hasActuallyClaimedSomething = testMode
    ? hasAlreadyClaimed
    : actualClaimStatus.hasClaimedAny;
          ...
          {claimState === "select" && (
            <>
              ...
              <div className="p-6 pt-4 space-y-4 w-full">
                ...
                >
                  Opt-in to auto-like/recast our daily winner announcements &
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="flex flex-col items-center p-3 rounded-lg border border-border">
                      ...
                      >
                        <span
                          className={cn(
                            "inline-block h-4 w-4 transform rounded-full transition-transform items-center justify-center",
                            likesEnabled
                ...
                >
                  ...
                  >
                    ...
                    {testMode
                      ...
                      : pendingClaim?.amount?.toLocaleString() ||
                        claimAmount.toLocaleString()}{" "}
                    $QR
                  </Button>
                </motion.div>
          ...
          {claimState === "success" && (
            <>
              ...
              <div className="p-6 pt-4 space-y-4 w-full">
                ...
                >
                  {claimAmount.toLocaleString()} $QR has been sent to your
                  wallet.
                </motion.p>
