 ...
 */
export function getAuctionImage(auctionId: number | string, defaultImage?: string): string {
  const id = typeof auctionId === 'number' ? auctionId.toString() : auctionId;
  return auctionImageOverrides[id] || defaultImage || `${String(process.env.NEXT_PUBLIC_HOST_URL)}/opgIMage.png`;
} 